...
# ====== Color helper function, essentially give jet color map in a convenient form ====
    ...
    def color_init(self, Fmax=3.):
        ...
        cl = ([0, (0., 0., 0.5)], [0.5, (0., 0., 1.)], [1., (0., 0.5, 1.)],
              [1.25, (0.5, 0.75, 0.5)], [1.7, (1., 0.5, 0.)],
              [2.0, (1., 0., 0.)], [Fmax, (0.5, 0., 0.)])
        n = len(cl)
        ...
        cm = {
            'red': [(l[0] / Fmax, l[1][0], l[1][0]) for l in cl],
...
# ===== Plotting force and Contact related pieces
    ...
    def plotStresses(self, plotCir, plotVel, plotCon, plotF, plotStress,
                     **kwargs):
        if 'figure' in kwargs:
        ...
        if 'timestamp' in kwargs:
            axval.text(0.0,
                       0.4 * self.Ly,
                       'T= ' + str(kwargs['timestamp']),
                       fontsize=18)
        for k in range(self.N):
            # Particle circles
            if (plotCir):
                cir1 = ptch.Circle((self.conf.x[k], self.conf.y[k]),
                                   radius=self.conf.rad[k],
                                   ec=(0.5, 0.5, 0.5),
                                   fc='none',
                                   linewidth=2)
                ...
                if self.verbose:
                    axval.text(self.conf.x[k],
                               self.conf.y[k],
                               str(k),
                               fontsize=8)
            # Velocity vectors
            if (plotVel):
                scale = 2 * np.mean(self.conf.rad) / np.sqrt(
                    np.mean(self.conf.dx * self.conf.dx) +
                    np.mean(self.conf.dy * self.conf.dy))
                velvect = ptch.Arrow(self.conf.x[k],
                                     self.conf.y[k],
                                     scale * self.conf.dx[k],
                                     scale * self.conf.dy[k],
                                     width=0.3,
                                     color='b')
        ...
        if (plotCon):
            for k in range(len(self.conf.I)):
                ...
                if self.verbose:
                    if (k >= self.ncon):
                        axval.text(
                            x0 + 0.5 * (x1 - x0) - self.small * np.sin(ang),
                            y0 + 0.5 * (y1 - y0) + self.small * np.cos(ang),
                            str(k),
                            fontsize=8)
                    else:
                        axval.text(x0 + 0.5 * (x1 - x0),
                                   y0 + 0.5 * (y1 - y0),
                                   str(k),
                                   fontsize=8)
        ...
        if (plotF):
            ...
            fscale = self.fgiven
            Fcolor, Fmap = self.color_init(
                np.sqrt(np.amax(self.conf.fnor)) / fscale)
            for k in range(len(self.conf.I)):
...
#def plotPebbles(self,plotCir,plotPeb,plotPebCon,plotClus,plotOver,axval,running):
    ...
    def plotPebbles(self, plotCir, plotPeb, plotPebCon, plotClus, plotOver,
                    **kwargs):
        if 'figure' in kwargs:
            fig = plt.figure(figsize=(20, 20))
            #fig=plt.figure()
            ...
            if (plotPeb):
                for j in range(3):
                    if (self.pebbles.pebbles[k, j] == -1):
                        ...
                        if (t == 3):
                            color = 'b'
                        cir2 = ptch.Circle((self.conf.x[k], self.conf.y[k]),
                                           radius=0.75 * self.conf.rad[k],
                                           ec=(0.5, 0.5, 0.5),
                                           fc=color,
                                           linewidth=2)
        ...
        if (plotClus):
            ...
            for k in range(self.pebbles.ncon2):
                if (self.pebbles.cluster[k] >= 0):
                    ...
                    try:
                        rgbclus[k, :] = self.random_read[
                            self.pebbles.cluster[k].astype(int), :]
                    except:
                        rgbclus[k, :] = rgbpattern[
                            self.pebbles.cluster[k].astype(int), :]
                        #print(rgbclus[k,:])
                else:
            ...
            for k in range(len(self.pebbles.Ifull)):
                ...
                x0, x1, y0, y1 = self.conf.getConPos2(self.pebbles.Ifull[k],
                                                      self.pebbles.Jfull[k])
                if (self.pebbles.cluster[k] != -1):
                    if (k >= self.ncon):
                        ang = np.arctan((y1 - y0) / (x1 - x0))
                        conlink = lne.Line2D([
                            x0 - self.small * np.sin(ang),
                            x1 - self.small * np.sin(ang)
                        ], [
                            y0 + self.small * np.cos(ang),
                            y1 + self.small * np.cos(ang)
                        ],
                                             linewidth=3,
                                             color=(rgbclus[k, 0],
                                                    rgbclus[k, 1], rgbclus[k,
                                                                           2]))
                    else:
                        conlink = lne.Line2D([x0, x1], [y0, y1],
                                             linewidth=3,
        ...
        axval.set_title('System with ' + str(self.pebbles.freepeb) +
                        ' free pebbles and ' + str(self.pebbles.fail) +
                        ' failed contacts')
        # returning figure pointer for use outside (like plotting or saving)
        if 'figure' in kwargs:
            return fig
...
# ============== Cluster statistics ===================
    ...
    def clusterStatistics(self):
        ...
        if (len(islargest) > 0):
            ...
            xval = np.sort(
                np.union1d(self.conf.x[Iarr[islargest]],
                           self.conf.x[Jarr[islargest]]))
            ...
            yval = np.sort(
                np.union1d(self.conf.y[Iarr[islargest]],
                           self.conf.y[Jarr[islargest]]))
    ...
    def crossCorrelate(self):
        ...
        if self.conf.hasAngles:
            ...
            dijtang = drijtan - (
                self.conf.rad[self.conf.I] * self.conf.dalpha[self.conf.I] +
            ...
            dtang_mob_signed = np.mean(dijtang[ismobi] *
                                       np.sign(self.conf.ftan[ismobi]))
            dtang_not_signed = np.mean(dijtang[notmobi] *
                                       np.sign(self.conf.ftan[notmobi]))
    ...
    def RigidDisplacementsCorrelate(self, minThresh, plotDisp=True):
        ...
        if self.conf.hasAngles:
            ...
            disp2n, disp2t, intThresh = self.conf.Disp2Contacts(
                minThresh, True)
        for k in range(self.ncon):
            if self.pebbles.cluster[k] > -1:
                Corr_PGDM[0] += 1.0
                if self.conf.hasAngles:
                    if (disp2r[k]) < intThresh and (disp2n[k] +
                                                    disp2t[k]) < intThresh:
                        Corr_PGDM[1] += 1.0
                else:
        ...
        if (plotdis):
            ...
            for k in range(self.N):
                ...
                if (duse > 3.0 * D2_scale):
                    duse = 3.0 * D2_scale
                cir1 = ptch.Circle((self.conf.x[k], self.conf.y[k]),
                                   radius=0.5 * radscale * duse / D2_scale,
                                   ec=Fcolor(duse / D2_scale),
                                   fc=Fcolor(duse / D2_scale),
                                   linewidth=2)
                axval.add_patch(cir1)
